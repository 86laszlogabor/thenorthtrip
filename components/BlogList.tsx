      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        {posts.map((p) => (
          <Link key={p.slug} href={`/blog/${p.slug}`} className="block">
            <Card className="p-5 hover:bg-slate-100 transition">
              ...
              <div className="mt-3 text-slate-900 font-semibold leading-snug">
                {p.title}
              </div>
              ...
              {p.tags?.length ? (
                <div className="mt-4 flex flex-wrap gap-2">
                  {p.tags.slice(0, 3).map((t) => (
                    <span
                      key={t}
                      className="rounded-lg border border-slate-200 bg-white/60 px-2 py-1 text-[11px] text-slate-700"
                    >
                      {t}
                    </span>
                  ))}
