...
export const metadata = {
  ...
  description:
    "Choose your Lapland base by decision points: tour availability, vibe, logistics, and winter recovery options. Fast routing into hub pages.",
  openGraph: {
...
function MiniTag({ icon, label }: { icon: React.ReactNode; label: string }) {
  return (
    <span className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-semibold text-brand-text/70">
    ...
    <Card className="flex flex-col overflow-hidden">
      ...
      {hub.tags?.length ? (
        <div className="mt-4 flex flex-wrap gap-2">
          {hub.tags.map((t) => (
            <MiniTag key={t.label} icon={t.icon} label={t.label} />
          ))}
        </div>
      ) : null}
      ...
      <div className="mt-auto pt-6">
        <Link href={hub.href} className="inline-flex items-center gap-2 text-sm font-semibold hover:underline">
          Open hub page <ArrowRight className="h-4 w-4" />
        </Link>
      </div>
    </Card>
  );
