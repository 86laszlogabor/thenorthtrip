...
const POSTS: Post[] = [
 ...
 {
 title: "Helsinki Airport â†’ Lapland Transfers: Shuttles & Tours (2026)",
 description:
 ...
 description:
 "Levi as a base: what itâ€™s great for, what itâ€™s not, and how to plan winter days without stacking fatigue.",
 ...
 description:
 "Rovaniemi as a: tours, Santa friction, aurora strategy, and what to skip when daylight is short.",
 category: "Destinations",
 ...
 description:
 "Whatâ€™s free vs paid at Santa Village, plus aurora logic that doesnâ€™t depend on luck cosplay.",
 category: "Destinations",
 ...
 description:
 "A clean Helsinki day trip that actually works: old town, riverfront pacing, and why itâ€™s a mental Lapland warm-up.",
 category: "Day trips",
 ...
 description:
 "City stops that actually add value, not filler. Decision-first pacing between bases.",
 category: "Cities",
 ...
 {
 title: "Siida SÃ¡mi Museum Inari: Culture Before Aurora Tours (2026)",
 description:
...
export default function BlogClient() {
 ...
 Fewer surprises, fewer â€œweâ€™ll figure it outâ€ moments. The guides stay factual and practical.
 </p>
 </div>
 ...
 {pageItems.length === 0? (<div className="mt-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
 <p className="text-sm text-slate-700">No matches.</p>
 </div>): (<div className="mt-6 grid gap-4 md:gap-6 sm:grid-cols-2 lg:grid-cols-3">
 {pageItems.map((p) => (<Card
 key={p.href}
 ...
 <p className="text-xs text-brand-text/60">
 Showing {(safePage - 1) * perPage + 1}â€“{Math.min(safePage * perPage, filtered.length)} of{" "}
 {filtered.length}
 </p>
 ...
 active? "border-slate-900 bg-slate-900 text-white": "border-slate-200 bg-white text-slate-800 hover:border-slate-300",
 ].join(" ")}
 >
 {n}
 </button>);
 })}
 {totalPages > 6? <span className="px-2 text-xs text-brand-text/60">â€¦</span>: null}
 </div>
 ...
 <span className="mx-2 text-brand-text/30">â€¢</span>
