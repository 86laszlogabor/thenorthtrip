...
const traps = [
 ...
 { label: "Missing one connection is always recoverable", icon: "??" },
 { label: "Winter only affects comfort, not viability", icon: "??" },
...
export default function HelsinkiAirportTransfersPage() {
 ...
 {seasonal.map((r) => (<tr key={r.aspect} className="border-b border-slate-200 last:border-b-0">
 ...
 {traps.map((t) => (<Pill key={t.label} icon={t.icon}>
 {t.label}
 </Pill>))}
 </div>
 ...
 <tbody className="text-brand-text/70">
 {decisionRows.map((r) => (<tr key={r.situation} className="border-b border-slate-200 last:border-b-0">
